...
def forwardFT( image ): #~four lines of code two loops - f(u, y), f(u, v)

  # YOUR CODE HERE
  #
  # You must replace this code with your own, keeping the same function name and parameters.
  row, col = image.shape
  FTimage = np.empty((row, col), dtype=complex)
  
  for j in range(row):
    FTimage[j,:] = ft1D(image[j,:])
  
  for i in range(col):
    FTimage[:,i] = ft1D(FTimage[:,i])
...
def inverseFT( image ): #one line of code use conjugate of F(u,v) to the forward 2D FT then conjugate of output. use np.conj function

  # YOUR CODE HERE
  #
  # You must replace this code with your own, keeping the same function name and parameters.
  row, col = image.shape
  return np.conj(forwardFT(np.conj(image)))/(row * col)
...
def multiplyFTs( image, filter ): #pull out e^ik(theta) from FT shifted function. we get e^ik(theta) times the FT of OG function to shift it. in discrete it is e^(ik2pi/N)theta. in 2D we do 2 multiplies one for u direction, one for v direction. e^{2 pi i x (N/2)/N} and e^{2 pi i y (M/2)/M}. combine to get (e^-ipi)^u+v = (-1)^(u+v). can make an image that is product of two and then multiplying by the -1^(u+v) term (or matrix of alternating 1 and -1's).

  # YOUR CODE HERE
  row, col = filter.shape
  FTproduct = np.empty((row, col), dtype=complex)
  filter_copy = np.copy(filter)
  
  #dont evaluate -1^(u+v)
  #if u+v is even its + otherwise its -
  #copy filter and negate the elements where it's negative
  #use parity based on row
  
  parity = 0
  for u in range(row):
    for v in range(parity, col, 2):
        filter_copy[row-1-u,v] = (-1) * filter[row-1-u,v]
    if parity == 0:
        parity = 1
    else:
        parity = 0
  ...
  for u in range(row):
    for v in range(col):
        FTproduct[u,v] = filter_copy[u,v] * image[u,v]
...
def modulatePixels( image, x, y, isFT ):
  ...
  # YOUR CODE HERE
  #numpy log, arctan2, sqrt, absolute all permitted
  #gaussian(d) = e ^(-d^2/2(sd)^2)
  #M' = |image(x,y)| ^ F where F is either 1-G or 1 + 1/10*G
  #this is because the e^ and the log cancel
  #for FT images find the symmetric point and adjust it as well
  row, col = image.shape
  sd = float(radius)/2.0
  for i in range(int(x-radius), int(x+radius)):
    for k in range(int(y-radius), int(y+radius)):
        dist = np.sqrt((x-i)**2 + (y-k)**2)
        #print(dist)
        if dist <= radius:
            gaussian = math.exp((-dist**2)/(2*(sd)**2))
            if isFT:
                row_ind = k
                col_ind = i
                if i < 0:
                ...
                if k < 0:
                    ...
                    row_ind = k - row
                magnitude = np.absolute(image[row_ind,col_ind])
                print("before:", magnitude)
                #print(gaussian)
                phase = np.arctan2(np.imag(image[row_ind,col_ind]), np.real(image[row_ind,col_ind]))
                if editMode == 'a':
                    magnitude = magnitude**(1 + 0.1*gaussian)
                elif editMode == 's':
                    magnitude = magnitude**(1 - gaussian)
                else:
                    print ("error not a valid editMode. edit options are: 'a' or 's'")
                print("after", magnitude)
                ak = magnitude * np.cos(phase)
                bk = magnitude * np.sin(phase)
                new_val = ak + bk*1j
                image[row_ind,col_ind] = new_val
                image[row-1-row_ind, col-1-col_ind] = new_val
            else:
                magnitude = image[k,i]
                if editMode == 'a':
                    magnitude = magnitude * (1 + 0.1*gaussian)
